...
import {
  ...
  Skills,
  TabContent,
} from "../../commoncontent/AboutusData";
          ...
          <div className="aboutus-right">
            <div className="aboutus-right-inside">
                ...
                {TabContent?.map((item, index) => {
                  return (
                    <div
                      key={index}
                      className={
                        item?.name === selectName ? "activeTab" : "inactiveTab"
                      }
                      onClick={() => handleAboutusTab(item?.name)}
                    >
                      {item?.name}
                    </div>
                  );
              ...
              <div className=" d-flex justify-content-center mt-4 inside-about-section-lefts">
                {selectName == "Personal Info" && (
                  <div>
                    <div className="decsription">{PersonalContent?.desc}</div>
                ...
                {selectName == "Qualifications" && (
                  <div>
                    <div
                      className="bolt-awesome mb-4 mt-2"
                      data-aos="fade-up"
                    ...
                    <div className="qualification-section row">
                      {Qualifications?.map((item, index) => {
                        return (
                          <div
                            className="qulification-box mb-3 mt-3"
                            key={index}
                          >
                            <div className="box">
                            ...
                            <div className="list-educations">
                              <div className="matername">{item?.name}</div>
                              <div className="matername">{item?.dgname}</div>
                              <div className="clgname">{item?.clgName}</div>
                              <div className="clgnames d-flex gap-2">
                                <span>{item?.calicon}</span> {item?.year}
                              </div>
                              <div className="clgname">{item?.place}</div>
                            </div>
                          </div>
                        );
                      })}
